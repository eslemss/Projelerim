// --- Sınıflar ---
class Kitap {
    constructor(id, ad, yazar, kapak, durum = 'Rafta') {
        this.kitap_id = id;
        this.ad = ad;
        this.yazar = yazar;
...
// --- Formlar ---
document.getElementById('kitapForm').onsubmit = async function(e) {
    e.preventDefault();
    ...
    if(ad && yazar) {
        const success = await kitapEkle(ad, yazar, kapak);
        if (success) {
...
// --- API Fonksiyonları ---
async function fetchKitaplar() {
    try {
        const response = await fetch(`${API_URL}/items`);
        const data = await response.json();
        kitaplar = data.map(item => new Kitap(
            item.id,
            item.title,
...
async function kitapEkle(ad, yazar, kapak) {
    try {
        const response = await fetch(`${API_URL}/items`, {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                title: ad,
                description: yazar,
                kapak: kapak || 'https://via.placeholder.com/90x130?text=Book'
            }),
        });
